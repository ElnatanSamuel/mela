    ...
    <div className="fixed inset-0 z-[200] flex items-end justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="relative h-72 bg-stone-100 shrink-0">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center text-white">
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="text-2xl font-black text-stone-900 tracking-tighter shrink-0">{formatCurrency(item.price)}</span>
          </div>
          ...
          {(item.description || item.descriptionAm) && (
            <div className="space-y-1">
              ...
              {item.descriptionAm && <p className="text-sm text-stone-300 leading-relaxed">{item.descriptionAm}</p>}
            </div>
          )}
          ...
          <div className="border-t border-stone-100 p-6 shrink-0">
            {cartQty > 0 ? (
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2 bg-stone-100 rounded-full p-1.5">
                  <button onClick={onRemove} className="w-11 h-11 rounded-full bg-white border border-stone-200 flex items-center justify-center active:scale-95 transition-transform shadow-sm">
                    <Minus className="w-4 h-4 text-stone-700" />
                  </button>
                  <span className="w-8 text-center text-lg font-black text-stone-900">{cartQty}</span>
                  <button onClick={onAdd} className="w-11 h-11 rounded-full bg-stone-900 text-white flex items-center justify-center active:scale-95 transition-transform shadow-lg">
                    <Plus className="w-4 h-4" />
                  </button>
                </div>
                <button onClick={onClose} className="flex-1 bg-stone-900 text-white py-4 rounded-full text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 active:scale-[0.98] transition-transform shadow-lg">
                  <ShoppingCart className="w-4 h-4" />
                  In Cart — {formatCurrency((parseFloat(item.price) * cartQty).toString())}
                </button>
              </div>
            ) : (
              <button onClick={onAdd} className="w-full bg-stone-900 text-white py-4 rounded-full text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 active:scale-[0.98] transition-transform shadow-lg">
                <Plus className="w-4 h-4" />
